...
void del_me()
{
	...
	me->delete_temp("hanzai_cond");
	me->set("job_done", "fleet_job");
	me->add("job/fleet", 1);
...
void new_game(object me)
{
	...
	me->set_temp("haizai_cond", "waiting_go");
	write("\n============ 设置成功！正在进入游戏…… ============\n");
...
int do_game(string arg)
{
	...
	if ( !me->query_temp("haizhan_cond") ) return 0;
...
int do_go(string arg)
{
	...
	if ( me->query_temp("haizhan_cond") != "ok" ) return notify_fail("现在不许移动。正在： "+ game_now + " 。\n");
	...
	if ( here == "岛屿" ) {
		...
		if ( random(100) > 80 ) {
			...
			obj->set("target", (string)me->query("id"));
	...
	if ( here == "望远镜" ) {
		...
		me->set_temp("haizhan_cond", "wangyuanjing");
		return 1;
	}
	if ( here == "报警探索仪" ) {
		map();
	...
	if ( here == "炮艇" || here == "驱逐舰" || here == "巡洋舰" || here == "潜艇" || here == "吉野号" ) {
		...
		me->set_temp("haizhan_cond", "fighting");
		att_now = "not_ok";
		map();
...
void check_hp()
{
	...
	if ( random( (int)me->query_skill("swimming", 1) ) < 80 ) {
		me->set_temp("last_damage_from", "坐舰被击沉，淹");
		me->die();
	}
	else {
...
int do_liaowang( string arg )
{
	...
	if ( me->query_temp("haizhan_cond") != "wangyuanjing" ) return 0;
...
void target_check_hp()
{
	...
	if ( target_hp < 1 ) {
		...
		if ( query("who_name") == "吉野号" ) {
			...
			reward = TASK_D->get_reward(me, 5000, 3, 1000 + random(500), me->query("job/fleet"));
			...
			msg = "【闲聊】北洋水师(beiyang shuishi)：" + me->query("name") + HIC"成功地击沉了日本海军旗舰──吉野号。\n";
		...
		if ( query("sunk_enemy") > 10 ) {
			...
			reward = TASK_D->get_reward(me, 1500, 3, 500 + random(500), me->query("job/jiawu"));
			exp = reward["exp_reward"];
			pot = reward["pot_reward"];
			me->add("combat_exp", exp);
			me->add("potential", pot);
			me->add("job/jiawu", 1);
			tell_object(me, "你被奖励了" + exp + "点经验值，" + pot + "点潜能。\n");
			msg = "【闲聊】北洋水师(beiyang shuishi)：" + me->query("name") + HIC"成功地击沉了日本海军旗舰的攻击。\n";
			message("channel:chat", HIC + msg + NOR, users());
			remove_call_out("del_me");
			call_out("del_me", 1);
			return;
