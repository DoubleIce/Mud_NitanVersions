...
{       
  	...
  	add_action("do_exchange", "exchange");
        add_action("do_exchange2", "duihuan");
}
...
int do_exchange(string arg)
{
           ...
           || arg == "蚩尤诛元仙丹")
	{
           ...
           || arg == "研究勋章")
        {
                bugp = 10;
                if (arg == "研究勋章")
                        ob = new("/clone/item/research-emblem");
                else
                if (arg == "学习勋章")
                        ob = new("/clone/item/learn-emblem");
                else
                if (arg == "汲取勋章")
                        ob = new("/clone/item/derive-emblem");
                else
                if (arg == "练习勋章")
                        ob = new("/clone/item/practice-emblem");
                else
                if (arg == "八十万黄金")
                {
                        ob = new("/clone/money/gold");
                        ob->set_amount(800000);
                }
        } else
        if (arg == "玄天玲珑锦")
        {
                bugp = 30;
                ob = new("/d/item/wizobj/xuantianlinglongjin");
        } else
        if (arg == "寰宇天晶")
        {
                bugp = 50;
                ob = new("/clone/item/tianjing");
        } else
        if (arg == "无名之物")
        {
                bugp = 80;
                ob = new("/clone/item/noname");
        } else
        if (arg == "符文")
        {
                bugp = 20;
                ob = new("/clone/enchase/rune33");
        } else
     	if (arg == "脑白金")
	{
		bugp = 100;
...
int do_exchange2(string arg)
{
        object ob, me = this_player();
        int point, pointp;

        point = me->query("nitan_paodian", 1);
        ...
        {
                pointp = 120;
                ob = new("/clone/item/shenmigift");
        } else
                return notify_fail(CYN "店小二对你摇摇头道：你只能兑换"
                                   "规定范围内的物品。\n" NOR);

        if (point < pointp)
                return notify_fail(CYN "店小二对你摇摇头道：你的泥潭泡点"
                                   "数不够，无法兑换此类物品。\n" NOR);

        me->add("nitan_paodian", -pointp);
        ...
        CHANNEL_D->do_channel(this_object(), "rumor", sprintf("听说%s用泥潭泡点在"
                              "店小二处换取一%s%s" HIM "。" NOR, me->name(),
                              ob->query("unit"), ob->name()));
