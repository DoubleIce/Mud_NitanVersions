...
int do_answer(string arg)
{
        ...
        if( arg == "yes" ) {
                ...
                room = load_object("/d/maze/shadow/maze/entry");
                me->move(room);
                tell_object(me, HIR "你来到幻境。。\n" NOR);
                me->set_temp("xinmo/to_kill", 1);
...
int do_report(string arg)
{
        ...
        if( !me->query_temp("xinmo/to_kill") )
                return notify_fail(BLU "泰山府君皱眉道：「你想要干什么？」\n" NOR);

        if( !me->query_temp("xinmo/killed") )
