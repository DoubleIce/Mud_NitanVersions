...
int do_donate(string arg)
{
        ...
        gold = count_mul(money, 10000);
        if( count_lt(me->query("balance"), gold) )
                return notify_fail(BLU "衡山府君皱眉道：「你银行存款根本没有这么多黄金。」\n" NOR);
        
        me->set("balance", count_sub(me->query("balance"), gold));
