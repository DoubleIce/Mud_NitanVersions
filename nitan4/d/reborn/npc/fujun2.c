...
int do_wager(string arg)
{
        ...
        if( !me->query_temp("can_wager") )
                return notify_fail(BLU "华山府君皱眉道：。。。\n" NOR);
        
        if( me->query_skillo(arg, 1) < 2000 )
...
int do_guess(string arg)
{
        ...
        if( random(10) < 5 ) {
                ...
                if( arg == "front" ) {
                        me->add_temp("guess/win", 1);
                        if( me->query_temp("guess/win") < 2 )
                                message_vision(BLU "$N" BLU "道：「你胜了一趟。」\n" NOR, ob, me);
                        else if( me->query_temp("guess/win") == 2 )
