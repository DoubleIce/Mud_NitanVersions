	...
	foreach(member in team) {
			...
			if(mazedir!=MAZE_D->query_maze_dir(member))//不在同一个副本里
				continue;
                        ...
                        if(count_gt(member->query("combat_exp"),2000000000)) {
                                        ref_mar = 10000;
                                        ref_pot = 20000;
                                        ref_exp = 40000;
                        } else if (count_gt(member->query("combat_exp"),1500000000)) {
                                        ref_mar = 15000;
                                        ref_pot = 30000;
                                        ref_exp = 60000;
                        } else if (count_gt(member->query("combat_exp"),1000000000)) {
                                        ref_mar = 20000;
                                        ref_pot = 40000;
                                        ref_exp = 80000;
			}
...
void die() {
	...
	if(quest_index == 7) {
			...
			reset_eval_cost();
                        for (int i = 0; i < 3; i++) 
                        {
                                item = new("/clone/enchase/rune2" + random(3));
                        ...
                        {
                                if (random(6) == 0)
                                        item = new("/clone/enchase/yelgem" + (4 + random(3))); 
                                else
                                if (random(6) == 1)
                                        item = new("/clone/enchase/maggem" + (4 + random(3))); 
                                else
                                if (random(6) == 2)
                                        item = new("/clone/enchase/blugem" + (4 + random(3))); 
                                else
                                if (random(6) == 3)
                                        item = new("/clone/enchase/redgem" + (4 + random(3))); 
                                else
                                if (random(6) == 4)
                                        item = new("/clone/enchase/grngem" + (4 + random(3))); 
                                else
                                        item = new("/clone/enchase/skull" + (4 + random(3))); 
                                item->move(environment()); 
                        }  		

                        if (random(100) < 2)
                        {
                                item = new("/clone/enchase/rune25");
                                item->move(environment());
                        }
                        if (random(100) < 5)
                        {
                                item = new("/clone/enchase/rune24");
                                item->move(environment());
                        }
                        if (random(100) < 10)
                        {
                                item = new("/clone/enchase/rune23");
                                item->move(environment());
                        }
	}
        return ::die();
