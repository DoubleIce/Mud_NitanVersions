...
int ghost = 0;
int xuruo = 0;
nosave int PVP_DAMAGE_SCALE = 20;
nosave int PVP_WOUND_SCALE = 15;
...
varargs int set_status_xuruo(int n, int flag)
{
        ...
        if( query_temp("apply/avoid_xuruo_effect") )
                return 0;
...
varargs int receive_damage(string type, int damage, object who)
{
        ...
        // 装备系统附加属性
        if( who && who->query_temp("apply/add_damage") && !query_temp("apply/avoid_damage_effect") )
        {
        ...
        if (who && damage > 100 && who->query_temp("apply/add_xuruo"))
        {
                t = who->query_temp("apply/add_xuruo");
                if (random(150) < t)
                set_status_xuruo(5, 0);
        }

#if 1
        if (who && playerp(this_object()) && playerp(who) && 
            this_object()->is_fighting(who))
        {
                damage = damage * PVP_DAMAGE_SCALE / 100;
                if (query("reborn/times") && ! who->query("reborn/times"))
                        damage /= 2;
        }
#endif
...
varargs int receive_wound(string type, int damage, object who)
{
        ...
        if (who && playerp(this_object()) && playerp(who) && 
            this_object()->is_fighting(who))
        {
                damage = damage * PVP_WOUND_SCALE / 100;
                if (query("reborn/times") && ! who->query("reborn/times"))
                        damage /= 2;
        }
#endif
...
void unconcious()
{
        ...
        if (me->query_temp("special_skill/immortal")) return;
...
varargs void die(object killer)
{
        ...
        if (environment(me)->query("no_death") && userp(me))
        {
                ...
                set("jing", 0); set("eff_jing", 0);
                set("qi", 0);   set("eff_qi", 0);
                unconcious();
                return;
        }
        ...
        if (userp(me) || playerp(me))
        {
                ...
                // add by chenzzz，死亡保护
                if ((killer && userp(killer)) || (dob && userp(dob)))
                { 
                        me->set("die_protect/last_dead", time());               
                        duration = (int)me->query("die_protect/duration", 1);
        
                        if (count_gt(me->query("combat_exp"), 20000000) && duration > 0)
                        {
                                duration = duration * 2;
                        }
                        else
...
int heal_up()
{
            ...
            (! stringp(my["doing"]) && interactive(me) || my["doing"] == "scheme"))
        {
                ...
                t = (my["con"] + my["max_jingli"] / 100) / scale;
                my["jing"] += t;
                ...
                if (my["jing"] >= my["eff_jing"])
                {
                        ...
                        if (my["eff_jing"] < my["max_jing"])
                        {
                                my["eff_jing"] ++;
                                // 装备系统附加属性
                                if (me->query_temp("apply/eff_jing"))
                                {
                                        q = me->query_temp("apply/eff_jing");
                                        my["eff_jing"] += q;
                                }
                                update_flag++;
                        }
                ...
                if (! me->is_busy())
                {
                        t = (my["con"] * 2 + my["max_neili"] / 200) / scale;
                        my["qi"] += t;
                }
                if (my["qi"] >= my["eff_qi"])
                {
                        ...
                        {
                                my["eff_qi"] ++;
                                if (me->query_temp("apply/eff_qi"))
                                {
                                        q = me->query_temp("apply/eff_qi");
                                        my["eff_qi"] += q;
                                }
                                update_flag++;
                        }
