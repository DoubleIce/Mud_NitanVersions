...
array query_killer() { return killer; }
array query_want()   { return want_kills; }
mapping query_default_action() { return ([ default_object : default_function ]); }
void    apply_killer(array ik) { if (arrayp(ik)) killer = filter_array(ik, (: stringp :)); }
void    apply_want(array ik)   { if (arrayp(ik)) want_kills = filter_array(ik, (: stringp :)); }
...
void want_kill(object ob)
{
        ...
        if (ob->query_temp("in_pkd")) return;
        ...
        if (userp(ob) && ! ob->query("no_newbie"))
        {
                me->set("no_newbie", 1);
                if (me->query("newbie"))
                {
