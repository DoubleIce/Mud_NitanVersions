...
void set_skill(string skill, int val)
{
        ...
        if (ob && userp(ob) && objectp(obj))
        {
                if (! is_root(obj) && ! obj->is_master())
                {
                        log_file("static/illegal",
                                 sprintf("%s  %s(%s) want to set %s skill(%s) to %d.\n",
                                         ctime(time()),
                                         (string)base_name(obj),
                                         (this_player(1) ? geteuid(this_player(1)) : "???"),
                                         (string)geteuid(ob), skill, val));
                }      
        }
#endif
...
void add_skill(string skill, int val)
{
        ...
        if (ob && userp(ob) && objectp(obj))
        {
                ...
                {
                        log_file("static/illegal",
                                 sprintf("%s  %s(%s) want to add %s skill(%s) to %d.\n",
                                         ctime(time()),
                                         (string)base_name(obj),
                                         (this_player(1) ? geteuid(this_player(1)) : "???"),
                                         (string)geteuid(ob), skill, val));
...
varargs int query_skill(string skill, int raw)
{
        if (! raw)
        {
                ...
                s = query_temp("apply/" + skill);
                p = query_temp("apply/add_skill");
                if (mapp(skills))
                {
                        s += (p + skills[skill]) / 2;
                        if (mapp(skill_map))
                                s += (p + skills[skill_map[skill]]); 
                }
                return s;
        }
        if (mapp(skills) && ! undefinedp(skills[skill]))
...
varargs void improve_skill(string skill, int amount, int weak_mode)
{
        ...
        case "martial":
                if (intp(mlvl = skills["martial-cognize"]) &&
                    skill != "martial-cognize")
                        ...
                        amount += atoi(count_div(count_mul(amount, (mlvl/5)), 100));
                break;
        ...
        lvl = skills[skill];
        amount = 1 + atoi(count_div(count_mul(amount, 100), (lvl + 100)));
