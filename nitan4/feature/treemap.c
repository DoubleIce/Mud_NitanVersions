...
protected nomask mixed _query( mapping map, array parts )
{
	mixed value;
	int i, s;
...
protected nomask int _delete( mapping map, array parts )
{
	if( sizeof( parts ) == 1 ) {
...
protected nomask mixed _set( mapping map, array parts, mixed value )
{
	if( sizeof( parts ) == 1 )
