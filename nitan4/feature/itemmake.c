...
int do_enchase(object me, object tessera)
{
        ...
        if (tessera->is_magic_item()) 
                return notify_fail("此类物品作用暂时没有开放，等待NEWS通知。\n");
        ...
        */
        ...
        if (intp(query("enchase/wash")))
                wash = query("enchase/wash");
        else
                wash = 0;
        ...
        for(int i = 0; i<sizeof(apply); i++)
                if (undefinedp(data[apply[i]]))
                {
                        ...
                        if (! this_object()->query("ultimate/ob") &&
        ...
        {
                addsn += random(tessera->query("enchase/SN")) + 1;
                if (addsn > 9) addsn = 9;
                if (addsn < 1) addsn = 1;

                add("enchase/SN", addsn);
        }
