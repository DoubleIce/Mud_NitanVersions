...
int perform_action(object me, string arg)
{
        ...
        if (target_ob)
        {
                avoid = me->query_temp("apply/avoid_locked"); 
                nopfm = target_ob->query_temp("apply/add_locked"); 
                if (me != target_ob && random(150) >= avoid && random(150) < nopfm)
