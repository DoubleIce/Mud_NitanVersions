...
protected int work_step_1(object me, object ob)
{
        ...
        if (objectp(ob) && ob->query("id") == "noname object")
        {
...
int do_notch(string arg)
{
        ...
        flag = 0;
        ob1 = present("bored water", me);
        if (objectp(ob1)) flag = 1;
        
        command("say 你要是有玄兵圣水这个开槽至宝就好了。");   
            ...
            ob->query("armor_type") == "hands"))
        {
                if (flag)
                {
                        ...
                        ob->set("enchase/flute", 3 + random(4));
        }
        else
        {
                if (flag)
                {
                        ...
                        ob->set("enchase/flute", 2 + random(3));
        }
                
        ob->set("enchase/used", 0);

        switch(ob->query("material"))
        {
        case "xuan yu"  :
        ...
        if( flag )
        message_vision(HIR "$N小心翼翼的将玄兵圣水撒在" + ob->name() + HIR "上，瞬间" +
                       ob->name() + HIR "变的透体发红。 \n" NOR, this_object(), me);
...
int do_wash(string arg)
{
        ...
        if (! wizardp(me) && (ob->is_item_make() && ob->item_owner() != me->query("id"))) 
        {
                command("kick " + me->query("id")); 
                command("say 你拿了别人的东西还敢来我这里？以为我是傻子？"); 
                return 1; 
        ...
        if (ob->query("enchase/rune30") || ob->query("enchase/rune31") || 
            ob->query("enchase/rune32") || ob->query("enchase/rune33"))
        {
                if (ob->query("skill_type") || (
                    ob->query("armor_type") && ob->query("armor_type") == "hands"))
                {
                        if (ob->query("enchase/used") >= 7)
                                ob->add("enchase/wash", 1);
                }
                else                
                {
                        if (ob->query("enchase/used") >= 5)
                                ob->add("enchase/wash", 1);
                }
        }

        f = ob->query("enchase/flute");
        w = ob->query("enchase/wash");
        ob->delete("enchase");
        ...
        ob->delete("ultimate/121");
        ob->delete("ultimate/ob");
        ob->set("enchase/flute", f); 
        ob->set("enchase/wash", w);
