...
public varargs int db_create_member(mixed ob, int money, string from_id)
{
        ...
        sql = sprintf("INSERT INTO %s SET id = \"%s\", money = %d, paytimes = 1, payinfo = \"%s\",
                      payvalue = %d, last_payvalue = %d, last_paytime = %d",
                      MEMBER_TABLE, id, money, payinfo, money, money, time());
...
public varargs mixed db_fee_member(mixed ob, int day, int flag)
{
        ...
        {
                jointime = time();
                if (day > 90) user->add("balance", "300000000");
                else user->add("balance", "100000000");
        }
...
public void show_member_info(mixed ob, string arg)
{
        ...
        // 面板
        case "info":
                write(BBLU + HIW "\t\t       王者归来会员系统面板\t\t     " + VERSION + "\n" NOR);
                write(HIW "≡---------------------------------------------------------------≡\n" NOR);
                write(HIY "WELCOME TO JOIN IN THE MEMBERS OF NITAN3 AND HOPE YOU ALL GOES WELL.\n\n" NOR);
                ...
                write(HIG "  *请使用" HIR " member zhuan amount to id " HIG " 转帐金币($NT)给别的角色(ID)。\n" NOR);
                ...
                write(HIR "  为避免带来不必要的损失请认真阅读泥潭会员制度文件" HIY "(help member)\n\n" NOR);
                write(HIY "                         NITAN3 WIZARD GROUP (http://www.ourmud.cn)\n" NOR);
                write(HIW "≡---------------------------------------------------------------≡\n" NOR);
