...
int do_enchase(object me, object item, object tessera)
{
        ...
        for(int i = 0; i<sizeof(apply); i++)
                if (undefinedp(data[apply[i]]))
                {
                        ...
                        if (! item->query("ultimate/ob") &&
                            (apply[i] == "research_effect" || apply[i] == "derive_effect" ||
                             apply[i] == "practice_effect"))
                                if (data[apply[i]] > 40) data[apply[i]] = 40;
                }                        
                else
                {
                        data[apply[i]] += enchase_prop[apply[i]];
...
mixed weapon10lv_hit_ob(object me, object victim, object weapon, int damage_bonus)
{
        ...
        case "lighting":
                ...
                if (damage < 0 || random(100) < avoid) return;
        ...
        case "fire":
                ...
                case 2:
                        ...
                        break;
                case 3:
                        msg = HIR + weapon->query("name") + HIB "暴出漫天通红的火球直击而来，$n"
                              HIR "被置身一片火海，惨叫连连。\n" NOR;
                        break;
                case 4:
        ...
        case "poison":
                // 毒系魔法：打内力或者忙乱
                resistance = victim->query_temp("apply/resistance/poison");
                damage = (power + jingjia) * 200 / (100 + resistance);
                add = me->query_temp("apply/add_poison");
                reduce = victim->query_temp("apply/reduce_poison");
                avoid = victim->query_temp("apply/avoid_poison");
                damage += damage * random(add + 1) / 100;
                damage -= damage * random(reduce + 1) / 100;
                ...
                case 0:
                        ...
                        break;
                case 1:
                        msg = HIG + weapon->query("name") + HIG "喷出股股黄烟快速围绕过来，$n"
                              HIG "顿时感觉头重脚轻，竟然险些拿不动兵器。\n" NOR;
                        break;
                case 2:
                        msg = HIG + weapon->query("name") + HIG "喷出漫天五彩迷雾扑面而来，$n"
                ...
                case 4:
                        ...
                        break;
                default:
                        msg = HIG + weapon->query("name") + HIG "喷出漫天浓香的五彩雾气，$n"
                              HIG "只觉醉生梦死，全身使不出力量来。\n" NOR;
                        break;
                }
                break;
        ...
        case "wind":
                // 风系魔法：打法力或者加命中
                resistance = victim->query_temp("apply/resistance/wind");
                damage = (power + jingjia) * 300 / (100 + resistance);
                add = me->query_temp("apply/add_wind");
                reduce = victim->query_temp("apply/reduce_wind");
                avoid = victim->query_temp("apply/avoid_wind");
                damage += damage * add / 100;
                damage -= damage * reduce / 100;
        ...
        extra = me->query_temp("apply/suck_qi");
        if (extra > 0)
        {
                victim->receive_damage("qi", extra, me);
                msg += HIR "$n" HIR "感到自己的气血被" + weapon->name() + HIR "吸干了似的。\n" NOR;
        }
        extra = me->query_temp("apply/suck_jing");
        if (extra > 0)
        {
                victim->receive_damage("jing", extra, me);
                msg += HIG "$n" HIG "感到自己的精气被" + weapon->name() + HIG "吸干了似的。\n" NOR;
        }
