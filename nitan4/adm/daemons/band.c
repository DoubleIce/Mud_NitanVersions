...
array Netclubs;
array Welcomes;
array Banned_IDs;                             //  ban id 表
array Banned_Names;                           //  ban 中文名表
...
array query_sites() { return Sites; }
array query_welcomes() { return Welcomes; }
array query_banned_ids() { return Banned_IDs; }
array query_banned_names() { return Banned_Names; }
...
#ifdef DB_SAVE
        if (! wizardp(user) && ! MEMBER_D->is_valid_member(user->query("id")))
#else
