...
varargs public void bonus(object who, mapping b, int flag)
{
        ...
        */
        ...
        if (count_gt(pot, 0)) msg += chinese_number(pot) + "点潜能、";
        if (count_gt(mar, 0)) msg += chinese_number(mar) + "点实战体会、";
...
varargs public void work_bonus(object who, mapping b, int flag)
{
        ...
        */
        ...
        if (gongxian > 0) msg += chinese_number(gongxian) + "点门派功绩、";
        if (gold > 0) msg += chinese_number(gold) + "两黄金、";
        if (silver > 0) msg += chinese_number(silver) + "两白银、";
...
varargs public void freequest_bonus(object who)
{
        ...
        quest *= 15;
        exp = random(quest) + quest + 5000;

        // 因为获得奖励较多且无法累及中断任务，所以暂时不做
        // 上限的限制。
        // pot = random(quest / 2) + 1000;
        pot = exp / 2;
        mar = random(quest / 2) + 1000;
