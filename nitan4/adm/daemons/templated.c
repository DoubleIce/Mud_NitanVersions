                             ...
                             int model, mapping function_data)
{
        ...
        if (file[<2..<1] != C_FILE_EXTENSION)
                file += C_FILE_EXTENSION;
                        ...
                        foreach (key in format)
                        {
                                ...
                                else
                                {
                                        if ((mapp(dbase[key]) && mapp(dbase_data[key])) ||
                                            (arrayp(dbase[key]) && arrayp(dbase_data[key])))
        ...
        // 配置 dbase 数据
        if (!arrayp(format = tob->query_dbase_format()) ||
            !sizeof(format))
                full = 1;
        else
                full = 0;
        ...
        if (! arrayp(format = tob->query_function_format()) ||
            ! sizeof(format))
                full = 1;
        else
                full = 0;
                           ...
                           int model, mapping function_data)
{
        ...
        array   dbase_format;   // dbase 格式
        array   function_format;// 函数格式
        string  temp_str1;
        string  temp_str2;
        ...
        if (tfile[<2..<1] != C_FILE_EXTENSION)
                tfile += C_FILE_EXTENSION;
                        ...
                        if (sizeof(dbase_data))
                        {
                                ...
                                ||  strsrch(temp_str1, sprintf("set\(\"%s/", key)) != -1)
                                {
                                        ...
                                        while(l < (n -1))
                                        {
                                                ...
                                                if (dbase_on && (strsrch(temp_str2, ";") != -1 ||
...
int receive_summon(object me)
{
        ...
        } else
        {
                if (env)
                {
                        ...
                        message("vision", "突然" + name() + "化作一道"
                                "光芒消失了！\n\n", env);

                        if (interactive(env = environment()))
                        {
                                tell_object(env, name() +
                                                 "忽然离你而去了！\n");
                        }
                }

                message_vision("一道光芒划过，只见$N"
                               "掌中多了一个$n！\n\n",
                               me, this_object());
        }
