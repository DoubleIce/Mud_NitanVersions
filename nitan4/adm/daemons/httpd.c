...
protected void close_callback(int fd)
{
	if ( fd == httpSock )
	{
		log_info(LOG_HTTP_ERR, "HTTP socket closed unexpectedly; restarting.\n");
		call_out("setup", 5);
	}
	else
	{
		...
		{
			log_info(LOG_HTTP_ERR, sprintf("Client socket %d closed unexpectedly\n", fd));
		}
		else
		{
			log_info(LOG_HTTP_ERR, sprintf("Client socket %s %d closed unexpectedly.\n",
...
void resolve_callback(string theName, string theAddr, int slot)
{
	...
	{
		log_info(LOG_HTTP_ERR, sprintf("Resolved %s to %s after connection closed.\n",
...
{
	string *parts, dir, ofile, args, id;
	mixed result;
	...
	{
		// automagically map .htm to .c files
		if ( !strsrch(file, DIR_WWW) && file[<4..<1] == ".htm" )
		{
			file = file[0..<5];

			if ( file_exists(file + ".c")
			     && catch(result = file->gateway()) )
			{
				log_http(fd, 400, 0, line0);
				http_error(fd, bad_gateway, "400 Bad Request");
				return;
