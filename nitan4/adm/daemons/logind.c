...
#define PUBLIC_IP       "203.166.160.43"
...
array banned_id = ({
        ...
        "all", "none", "cancel", "enter", "entry", "leave", "out", 
});
...
void logon(object ob)
{
        ...
        login_usr = filter_array(children(LOGIN_OB), (: query_ip_number($1) == $2 :), ip);

        // if ((sizeof(login_usr) > 20 && ip != PUBLIC_IP) || sizeof(login_usr) > 50)
        if (sizeof(login_usr) > 30)
        {
                blocks_ip(ip, 300, "您的IP产生了大量废弃连接！");
        ...
        if (ob->query_temp("tomud"))
                write(ALERT("经典文字MUD游戏『王者歸來』，群号『44135826』，本游戏为免费游戏！$$" +
                        "请到 http://www.mudzone.org 浏览本游戏主页和论坛、新闻。$$"+
        ...
        for (i = 0; i < sizeof(usr); i++)
        {
                ...
                if (MEMBER_D->is_valid_member(usr[i]))
                        vips += ({ ipname });
...
#ifndef MULTI_LOGIN
        if (iplimit > 4 && ip != PUBLIC_IP && 
            member_array(ip, vips) == -1 &&
            ! BAN_D->is_netclub(ip))
        {
        ...
        else
        {
                ...
                write("您的英文名字(" CYN "忘记密码请输入「pass」" NOR ")：\n");
        }
...
LONG ;
                ...
                MAIL_D->queue_mail(0, 0, arg, "王者归来 - " + ob->query("id") + " 角色密码", msg);        
	        // SMTP_D->send_passmail(0, 0, arg, "王者归来 - " + ob->query("id") + " 角色密码", msg);
        } else 
                write("您输入的资料与该角色不符。\n");
...
#ifdef NO_FEE
        if (ob->query_temp("tomud"))
                write(ALERT("文字MUD游戏『王者歸來』将不定期限免费开放！请新老玩家相互转告！\n"));
#else
        // 如果是月费用户，则不需要再判断点数了。
...
#ifdef DB_SAVE
        if (MEMBER_D->is_valid_member(user->query("id")) && ! BAN_D->is_netclub(query_ip_number(user)))
#else
