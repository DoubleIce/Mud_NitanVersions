...
public array query_ip_numbers() { return ip_numbers; }
#include <war.h>

array zhen1 = ({ "普通阵","鱼鳞阵","锋矢阵","鹤翼阵","偃月阵","方圆阵","雁行阵","长蛇阵","云龙阵" });
mapping zhen2 = ([
...
void init_player(object me)
{
        ...
        me->set_temp("warquest/time", time());
...
#ifdef DB_SAVE 
        &&  ! MEMBER_D->is_valid_member(me->query("id")) 
#endif
           )
                                        ...
                                        switch(ob->query_temp("warquest/purchase"))
                                        {
                                                case "weapon" :
                                                        message_vision(HIY "$N" HIY "率领的购置兵甲大军终于到达了襄阳军营，\n" +
                                                                "$N" HIY "的兵甲及时支援了大军的军备....\n" NOR, ob);
                                                        ob->add_temp("warquest/reward", 100);
                                                ...
                                                case "forage" :
                                                        ...
                                                        ob->add_temp("warquest/reward", 225);
...
void give_bouns()
{
        ...
        exp = reward * (song_info->marshal->query("degree_jungong") + 1) * 8;
        ...
        prompt = "因抗蒙有功";
        GIFT_D->delay_bonus(song_info->marshal,
        ...
        message("channel:rumor", HIY "【大宋京城】奉天承命，皇帝诏曰：\n" +
                "            " + song_info->marshal->name(1) + HIY "抗蒙有功，赏黄金百两，绸缎五百匹，美女十名，" +
        ...
        if (arrayp(total) && sizeof(total) > 0)
        {
                for (i = 0; i < sizeof(total); i++)
                {
                        ...
                        exp = reward * (total[i]->query("degree_jungong") + 1) * 8;
                        if (war_times) exp *= war_times;
                        pot = exp / 2;
                        mar = pot / 2;
                        jungong = exp / 1000;
                        ...
                        GIFT_D->delay_bonus(total[i],
                                ([ "prompt" : prompt, "exp" : exp, "pot" : pot, "mar" : mar ]));
                        ...
                        if (reward >= 2000 && time() - total[i]->query_temp("warquest/time") >= 3300)
                        {
                                ob = new("/clone/gift/puti-zi");
                                ob->set_amount(2);
                                ob->move(total[i], 1);
        ...
        for (i = 0; i < me->query_temp("warquest/group"); i++)
        {
                    ...
                    me->query_temp("warquest/train") == "infantry")
                {
                        ...
                        */
                        COMBAT_D->do_attack(me, target, me->query_temp("weapon"));
                        // 杀江洋大盗没有奖励
                        if (target->query_temp("warquest/party") == "meng")
                                me->add_temp("warquest/reward", 4);
                } else
