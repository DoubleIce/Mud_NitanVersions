...
// 不需要构造的目录
nosave array exclude_dir = ({ "/adm/etc", "/adm/simul_efun", "/adm/tmp",
                                "/data", "/dump", "/temp", "/binaries", VERSION_DIR,
                                "/backup", "/log", "/u", "/www" });
// 额外需要构造的文件
nosave array include_dir = ({ "/data/e2c_dict.o", "/data/emoted.o", "/data/newsd.o",
                                "/adm/etc/quest", "/adm/etc/preload", "/adm/etc/database.h" });

// 需要发布C源程序的目录
nosave array release_src = ({ "/clone/misc/wedding-ring.c",
                                "/clone/npc/pet.c",
