...
int main(object me, string arg)
{
           ...
           || sscanf(arg, "%s -m", arg)))
        {
                ...
                combat_exp = count_div(ob->query("combat_exp"), 100);
             	exp = atoi(combat_exp);               
                level = sqrt3(exp) * 10 + 1;	        
	        while (count_le(count_div(big_number_pow((level + 1), 3), 10), ob->query("combat_exp")))
                        level++;
                               
               	lv = ob->query("level");
               	if (lv < 1) lv = 1;
               	need = count_sub(count_mul(big_number_pow((lv + 1), 3), 10000), ob->query("combat_exp"));
                ...
                sp += sprintf(HIC "【潜能上限】 " HIG " %-21s"
                      ...
                      count_sub(ob->query_experience_limit(), ob->query("learned_experience")));
                ...
                sp += HIW "【死亡保护】  " NOR + sprintf("%-37s",
                      ...
                      count_lt(ob->query("combat_exp"), 20000000))) ? HIY "保护中" NOR :
                      HIY "无保护" NOR);

                sp += HIW "【杀戮保护】  " NOR + sprintf("%s",
                      (me->query("die_protect/last_dead") + me->query("die_protect/duration") < time()) ? HIY "无保护\n" NOR :
                      HIY "保护中\n" NOR);
        ...
        sp += sprintf(HIC "【 精 气 】 %s%5d/ %5d %s(%3d%%)"
                ...
                status_color(my["eff_jing"], my["max_jing"]),
                             atoi(count_div(count_mul(my["eff_jing"], 100), my["max_jing"])),
        ...
        sp += sprintf(HIC "【 气 血 】 %s%5d/ %5d %s(%3d%%)"
                ...
                status_color(my["eff_qi"], my["max_qi"]),
                             atoi(count_div(count_mul(my["eff_qi"], 100), my["max_qi"])),
        ...
        sp += sprintf(HIW "【 饮 水 】 %s%5d/ %5d      " HIW "     【 体 会 】  %s%s\n",
                status_color(my["water"], ob->max_water_capacity()),
                my["water"], ob->max_water_capacity(),
                count_ge(my["experience"], ob->query_experience_limit()) ? HIM : HIY,
                count_sub(my["experience"], my["learned_experience"]));
        ...
        sp += HIC "≡" HIY "───────────────────────"
              "────────" HIC "≡\n" NOR;
        sp += sprintf(HIR "【泥潭泡点】 " NOR + HIG " %6d 点\n", ob->query("nitan_paodian"));
        tell_object(me, sp);
        return 1;
}
