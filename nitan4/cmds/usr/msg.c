...
int main(object me, string arg)
{
        ...
        {
                if( !wizardp(me) && time() - me->query_temp("last_mail_msg") < 60 )
        ...
        {
                if( type == "say" || type == "tell" || type == "ask" || type == "reply" || 
                    type == "answer" || type == "whisper" || member_array(type, me->query("channels")) != -1)
                {
                        me->set("msg/"+type, 1);
                        write("开始纪录 "+type+" 的讯息。\n");
                        me->save();
...
void done(object me, string id, string msg)
{
        ...
        if( objectp(ob = find_player(id)) )
        {
                ...
                ob->add_msg_log("mail", HIR "【泥潭信使】来自" + me->query_idname() + HIR "的邮件内容如下\n"HIW+msg+NOR"\n"); 
                ob->save();                
                tell_object(ob, HIR "【泥潭信使】你新收到一封来自" + me->query_idname() + HIR "的邮件(msg mail)。\n" NOR);
        } else
        {
#ifdef DB_SAVE        
                ...
                msg = HIR "【泥潭信使】来自" + me->query_idname() + HIR "的邮件内容如下\n"HIW+msg+NOR"\n";
        ...
        tell_object(me, HIY "【泥潭信使】你的短消息已经成功发送！\n" NOR);
        return;
}
