...
void do_more(string arg,array text,int line,object ob);
...
varargs void view2(int line,array text,object ob,int i)
{	
	int w=line;
	if(i) w=w+17;
	write( YEL"－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－\n"NOR);  
...
void do_more(string cmd,array text,int line,object ob)
{
        int i,j,num;
        mapping *notes;
...
void view5(string arg)
{
	int k=atoi(arg),i,j;
	array w, text;
	string file,t=this_player()->query_temp("select");
	file=find_file();
	file=read_file(file);
	if(!file) {
		write(RED"没有链接文件!\n"NOR);
