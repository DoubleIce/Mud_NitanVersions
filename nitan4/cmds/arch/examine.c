...
protected string examine_player(string name, int copy_user, int raw, int last_touched)
{
        ...
        if (raw)
        {
                ...
                if (day >= UNLOG_IDLE && ! objectp(find_player(name))
#ifdef DB_SAVE
                && ! MEMBER_D->is_member(name)
#endif
                )
                {
...
protected string is_illegal(object ob)
{
        ...
        {
                mapping list = ([
                        "/clone/money/thousand-gold" : ({ 1000, 1, }),
                ...
                for (--i; i >= 0; i--)
                {
                        ...
                        gold = count_add(gold, count_div(count_mul(amount, val[0]), val[1]));
        ...
        dqst = ob->query("quest_count") - ob->query("last_examine/quest_count");
        dexp = count_sub(exp, ob->query("last_examine/combat_exp"));
        dpot = count_sub(ob->query("potential"), ob->query("last_examine/potential"));
        dmar = count_sub(ob->query("experience"), ob->query("last_examine/experience"));
        ...
        if (dt > 60)
        {
                ...
                ob->set("last_examine/quest_count", ob->query("quest_count"));
                ob->set("last_examine/time", time());
                if (! ob->query_temp("user_setup"))
                        ob->save();
        }

        if (count_gt(gold, 100000))
                return HIY "拥有 " + gold + " 两黄金" NOR;

        /*
        if (count_lt(exp, 1000000) && count_ge(gold, 50000))
                return HIC "经验 1M 以内拥有 " + gold + " 两黄金" NOR;
        */

        if (count_gt(exp, 10000) && dt > 60)
        {
                if ((sp = atoi(count_div(count_mul(dexp, 60), dt))) > 8000)
                        return HIR "平均经验速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;

                if ((sp = atoi(count_div(count_mul(dpot, 60), dt))) > 4000)
                        return HIR "平均潜能速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;

                if ((sp = atoi(count_div(count_mul(dmar, 60), dt))) > 3000)
                        return HIR "平均体会速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;
                        
                if ((sp = atoi(count_div(count_mul(dqst, 60), dt))) > 6)
                        return HIR "平均任务速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;                        
        }
