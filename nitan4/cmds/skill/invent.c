...
int main(object me, string arg)
{
        ...
        if (! wizardp(me) && ! MEMBER_D->is_valid_member(me->query("id"))) 
...
BOOK;
        content += sprintf("\t\tset(\"long\", \"这是一本用薄绢写成的书。上书：“%s”。\\n书上署名%s(%s)。\\n\");\n",
                          skill_c_name, me->query("name"), me->query("id"));
        content += @BOOK
                set("value", 1);
                set("autoload", 1);
                set("material", "silk");
