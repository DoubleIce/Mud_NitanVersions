...
int main(object me, string arg)
{
	...
	if (! arg) return notify_fail("你要用外功做什麽？\n");
            ...
            sscanf(arg, "%s twice", arg) == 1 && (and = arg))
        {
                ...
                if (me->query_skill("count", 1) && ! me->query("special_skill/capture"))
                        return notify_fail("你杂学太多，心头烦乱，难以分心二用。\n");

                if (lvl <= 100)
                        return notify_fail("你的左右互搏修为太浅，难以分心二用。\n");

                if ((int)me->query("int") > 24 && (int)me->query("int") < 40)
                        return notify_fail("你试图分心二用，但是手就是不听使唤。\n");

                if ((! me->query("reborn/family_name") || 
                    (me->query("reborn/family_name") != "全真教" &&
                    me->query("reborn/family_name") != "古墓派")) && 
                    (! me->query_family() ||
                    ( me->query_family() != "全真教" &&
                      me->query_family() != "古墓派" )))
                        return notify_fail("你试图分心二用，但是手就是不听使唤。\n");
