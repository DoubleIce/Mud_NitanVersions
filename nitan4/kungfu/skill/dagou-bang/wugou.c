...
*/
        ...
        message_vision(HIW"$N手中的"+obj->name()+"微微抖动，将“打狗棒法”使得变幻莫测，一顿一挑看似笨拙，却有着无穷的威力！\n",me);
              ap = atoi(count_div(COMBAT_D->skill_power(me,"staff", SKILL_USAGE_ATTACK), 500)) + 1;  
              pp = atoi(count_div(COMBAT_D->skill_power(target,"parry", SKILL_USAGE_ATTACK), 1000)) + 1;  
              dp = atoi(count_div(COMBAT_D->skill_power(target,"dodge", SKILL_USAGE_ATTACK), 1000)) + 1;  
        num = me->query("wugou");
