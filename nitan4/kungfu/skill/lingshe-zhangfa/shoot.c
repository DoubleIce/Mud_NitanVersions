...
int perform(object me, object target)
{
	...
	exp2 = target->query("combat_exp");
        ...
        if(count_lt(count_mul(exp1,3), exp2) && count_gt(exp2, 1000000))
	...
	me->add("jingli", -50);
        ...
        if( me->query("neili") > random(target->query("neili")) && atoi(count_div(exp1, 100)) > random(atoi(count_div(exp2, 100))) ) {
