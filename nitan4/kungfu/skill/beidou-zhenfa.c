...
int form_array(object me,string arg)
{
	...
	if (enemy->query("race") != "人类")
	   ...
	   if ( member[i]->query_skill("quanzhen-jian",1) <60 || 
                count_lt(member[i]->query("combat_exp",1), 50000) ||
	   ...
	   if ( member[i]->query("jingli") < 300)
		return notify_fail("你队伍中的" + member[i]->name() + "精力太差，不能参加「天罡北斗阵」。\n");
           ...
           exp_diff = to_float(count_lt( member[i]->query("combat_exp" ), me->query("combat_exp"))); 
           exp_diff /= to_float(me->query("combat_exp"));
