...
int perform(object me, object target)
{
        ...
        ap = attack_power(me,"strike") + me->query("str") * 15;
        dp = defense_power(target,"parry") + target->query("con") * 15;
        dd = defense_power(target,"dodge") + target->query("dex") * 15;

        if (ap / 2 + random(ap) > dp)
        {
                damage = damage_power(me,"strike") * 2 / 3;
