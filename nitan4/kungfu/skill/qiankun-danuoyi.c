...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        if (ap / 3 + random(ap) < dp
           ...
           && me->query("max_neili") > random(ob->query("max_neili"))
           && sizeof(sk)
           && sizeof(ma) != 0
           && mapp(ma))
        {
        ...
        if (ap / 3 + random(ap) < dp || (me->query_temp("nuozhuan") && random(100) < 90))
        {
...
void nuoyi(object ob, object me, int damage)
{
        ...
        {
                msg += COMBAT_D->do_damage(me, ob, SPECIAL_ATTACK, damage, 300,
