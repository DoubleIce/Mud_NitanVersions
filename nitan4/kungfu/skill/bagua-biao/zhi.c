...
int perform(object me, object target)
{
        ...
        my_exp = count_add(me->query("combat_exp"), count_mul(count_div(count_mul(skill, skill), 10), skill));
        ob_exp = target->query("combat_exp");

        if (random(atoi(count_div(my_exp, 100))) > atoi(count_div(ob_exp, 100)))
        {
                n = 5 + random(5);
                if (count_gt(random(atoi(count_div(my_exp, 100))), count_div(ob_exp, 100))) n += 5 + random(5);
                if (count_gt(random(atoi(count_div(my_exp, 200))), count_div(ob_exp, 100))) n += 5 + random(5);
                if (count_gt(random(atoi(count_div(my_exp, 400))), count_div(ob_exp, 100))) n += 5 + random(10);
