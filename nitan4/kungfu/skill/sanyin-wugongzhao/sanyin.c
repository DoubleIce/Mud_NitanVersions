...
{
        object weapon;
        int damage, skill, merand, targrand, targneili,time,i;
        mixed myexp, targexp;
...
*/
        ...
        myexp = atoi(count_div(me->query("combat_exp"),100));
        targexp = atoi(count_div(target->query("combat_exp"), 100));
        skill = me->query_skill("huagong-dafa",1);
        
        merand   = random((myexp/100)*(me->query_skill("force")/10));
        targrand = (myexp/100)*(target->query_skill("dodge")/10)/3;
