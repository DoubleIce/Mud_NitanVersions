...
//              return notify_fail(target->name() + "目前正自顾不暇，放胆攻击吧ⅵ\n");
       ...
       if (count_lt(count_div(count_mul(me->query("combat_exp"),100),target->query("combat_exp")),120))
                a=3;
        else {
       if (count_lt(count_div(count_mul(me->query("combat_exp"),100),target->query("combat_exp")),150))
                        a=4;
                else {
       if (count_lt(count_div(count_mul(me->query("combat_exp"),100),target->query("combat_exp")),180))
                                a=5;
                        else a=6;
                }
        }
