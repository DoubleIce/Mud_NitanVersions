...
/*
        if(count_gt(count_div(me->query("combat_exp"), 100), me->query("shen")))
...
int second_hit(object me, object target){
        ...
        if(me->is_fighting()){
                ...
                ap=atoi(count_div(me->query("combat_exp"), 100))+me->query_skill("whip",1)+me->query("neili");
                if (living(target))
                      dp=atoi(count_div(target->query("combat_exp"), 100))+target->query_skill("whip",1)+target->query("neili");
                else  dp = 1;
