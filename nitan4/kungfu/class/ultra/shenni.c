...
int give_quest()
{
    ...
    */
    ...
    if(me->query("questsn_times") < 3
        || (me->query("questsn_times") > 5 && random(20) == 5))
    {
...
//    message_vision("$N将$n交给南海神尼。\n",me,ob);与give信息重复haiyan
    ...
    else GIFT_D->bonus(me, ([ "exp" : exp, "pot" : pot, "mar" : mar, "percent" : 80, 
                              "weiwang" : weiwang, "score" : score ]), 1);
   ...
   quest_count = me->query("questsn_times") + 1;
   ...
   me->set("questsn_times", quest_count);
   ...
   {
       obj = new("/clone/enchase/grngem.c");
