...
int give_quest()
{
    ...
    */
    ...
    if((time()-ob->query("questkh_giveup_time")) < (20+random(10)))
    {
        ...
        if(ob->query("questkh_times") < 3
        || (ob->query("questkh_times") > 5 && random(20) == 5))
        {
...
//   message_vision("$N将$n交给葵花太监。\n",who,ob);与give信息重复haiyan
   ...
   exp = level*300 + questkh["bonus"]*12;
   if (level == 9)
       exp += who->query("questkh_times")*3;
   else 
       exp += who->query("questkh_times")*2;
   exp = exp/i;
   ...
   else GIFT_D->bonus(who, ([ "exp" : exp, "pot" : pot, "mar" : mar, "percent" : 80, 
                              "weiwang" : weiwang, "score" : score ]), 1); 
   
   // who->add("questkh_times",1);
   quest_count = who->query("questkh_times") + 1;
   ...
   who->set("questkh_times", quest_count);
   ...
   {
       obj = new("/clone/enchase/blugem.c");
